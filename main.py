...
try:
    ...
    print(f"✗ Error during PyTorch import: {e}")
    print("PyTorch import interrupted - running in demo mode")
    TORCH_AVAILABLE = False
...
try:
    ...
    print(f"Warning: Nibabel not available: {e}")
    print("NIfTI file support will be limited")
    nib = None
...
try:
    ...
    print(f"Warning: Scikit-image not available: {e}")
    measure = None
    morphology = None
...
if TORCH_AVAILABLE:
    ...
    print("Running in demo mode - PyTorch models not available")
    segmentation_model = None
    classification_model = None
    device = None
...
def start_training():
    ...
    try:
        ...
        # Try to use the web training manager
        try:
            from web_training import start_web_training
            session_id = start_web_training(config)
            ...
            return jsonify({
                'success': True,
                'session_id': session_id,
                'message': 'Training started in demo mode'
            })
...
def stop_training():
    ...
    try:
        # In a real implementation, you would stop the training process here
...
def training_progress():
    ...
    try:
        ...
        # Try to get real training progress
        try:
            from web_training import get_web_training_progress
            progress_data = get_web_training_progress(session_id)
        ...
        progress = {
            ...
            'val_loss': round(random.uniform(0.1, 1.2), 4),
            'dice_score': round(random.uniform(0.7, 0.95), 4),
            'best_dice': round(random.uniform(0.85, 0.95), 4),
            'learning_rate': 0.0001,
            'logs': [
                f"Epoch {random.randint(1, 50)} completed",
                f"Validation dice improved to {random.uniform(0.8, 0.9):.4f}"
            ]
        }
...
def generate_synthetic_data():
    ...
    try:
        ...
        # In a real implementation, you would call your data generation function
        print(f"Generating {num_samples} synthetic samples...")
        
        return jsonify({
            'success': True,
            'num_samples': num_samples,
            'message': f'Generated {num_samples} synthetic BraTS samples'
        })
...
def train_main():
    print("Training function placeholder.  No training is actually performed.")

def main():
    print("=" * 60)
    print("Brain Tumor Segmentation System Starting...")
    print("=" * 60)
    ...
    try:
        ...
        print("Please check that port 5000 is available")
